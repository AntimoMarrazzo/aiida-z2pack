...
def prepare_wannier90(self,tempfolder,inputdict_wannier90):
    ...
    def _prepare_for_submission_wannier90(self,tempfolder, inputdict):        
        ...
        try:
            ...
            raise exceptions.InputValidationError("No parameters specified for "
                                       "this calculation")
        if not isinstance(parameters, orm.Dict):
            raise exceptions.InputValidationError("parameters is not of "
                                       "type orm.Dict")
        ...
        def check_capitals(input_params):
            ...
            for k in input_params:
                if k != k.lower():
                    raise exceptions.InputValidationError("Please make sure all keys"
        ...
        if not isinstance(precode_parameters,orm.Dict):
            raise exceptions.InputValidationError('precode_parameters is not '
                                       'of type orm.Dict')
        ...
        #except KeyError:
        #    raise exceptions.InputValidationError("No projections specified for "
        #                               "this calculation")
        #if not isinstance(projections, OrbitalData):
        #    raise exceptions.InputValidationError("projections is not of type "
        #                                 "OrbitalData")
        ...
        #except KeyError:
        #    raise exceptions.InputValidationError("No kpoints specified for this"
        #                               " calculation")
        #if not isinstance(kpoints, KpointsData):
        #    raise exceptions.InputValidationError("kpoints is not of type KpointsData")
        ...
        #     raise exceptions.InputValidationError("kpoints_path is not of type "
        #                                "KpointsData")
        ...
        try:
            ...
            raise exceptions.InputValidationError("No structure specified for this "
                                       "calculation")
        if not isinstance(structure, orm.StructureData):
            raise exceptions.InputValidationError("structure is not of type "
                                       "orm.StructureData")
        ...
        if settings is None:
            ...
            if not isinstance(settings,  orm.Dict):
                raise exceptions.InputValidationError("settings, if specified, must be "
                                           "of type orm.Dict")
        ...
        if preproc_code is not None:
            if not isinstance(preproc_code, orm.Code):
                raise exceptions.InputValidationError("preprocessing_code, if specified,"
                                           "must be of type orm.Code")
        ...
        if init_only:
            if preproc_code is None:
                raise exceptions.InputValidationError ('You cannot have init_only '
