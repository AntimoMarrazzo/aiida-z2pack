...
!
SUBROUTINE parity_band(filband, spin_component, firstk, lastk)
  !-----------------------------------------------------------------------
  !
  USE kinds,                ONLY : DP
  USE ions_base,            ONLY : nat, ityp, ntyp => nsp
  ...
  IMPLICIT NONE
  !
  INTEGER :: npw, ik, i, j, irot, iclass, ig, ibnd
  ...
  COMPLEX(DP),ALLOCATABLE :: times(:,:,:)
  REAL(DP) :: dxk(3), dkmod, dkmod_save, modk1, modk2, k1(3), k2(3), ps
  INTEGER, ALLOCATABLE :: rap_par(:,:), code_group_k(:)
  INTEGER, ALLOCATABLE :: ngroup(:), istart(:,:)
  CHARACTER(len=11) :: group_name
  CHARACTER(len=45) :: snamek(48)
  CHARACTER (len=256) :: filband, namefile
  ...
  DO ik = nks1, nks2
     ...
     IF (invsymk) THEN
           CALL find_parity_sym_so(ik, evc,et(1,ik),nsymk,sk,ftk,d_spink,&
                gk,invsk,rap_par(1,ik),times(1,1,ik),ngroup(ik),&
                istart(1,ik),accuracy)
     ENDIF
  ...
  !
  CALL ipoolrecover(code_group_k,1,nkstot,nks)
  CALL ipoolrecover(rap_par,nbnd,nkstot,nks)
  CALL poolrecover(times,2*24*nbnd,nkstot,nks)
  CALL ipoolrecover(ngroup,1,nkstot,nks)
  CALL ipoolrecover(istart,nbnd+1,nkstot,nks)
#endif
...
!
     DO ik=nks1tot, nks2tot
        ...
        CALL find_info_group(nsymk,sk,t_revk,ftk,d_spink,gk,snamek,&
             ...
             invs_is,is_symmorphic,search_sym)
        IF (code_group_k(ik) /= code_group) &
             CALL errore('sym_band','problem with code_group',1)
        WRITE(stdout, '(/,1x,74("*"))')
        WRITE(stdout, '(/,20x,"xk=(",2(f10.5,","),f10.5,"  )")') &
        ...
        IF (ik == nks1tot) THEN
           WRITE (iunout, '(" &plot_par nbnd_par=",i4,", nks_par=",i4," /")') &
                nbnd, nks2tot-nks1tot+1
           IF (search_sym) CALL write_group_info(.true.)
           dxk(:) = xk(:,nks1tot+1) - xk(:,nks1tot)
           dkmod_save = sqrt( dxk(1)**2 + dxk(2)**2 + dxk(3)**2 )
...
!
  ...
  !HERE TO CHANGE
        WRITE (iunout, '(10x,3f10.6,l5)') xk(1,ik),xk(2,ik),xk(3,ik), &
             high_symmetry(ik)
        WRITE (iunout, '(10i8)') (rap_par(ibnd,ik), ibnd=1,nbnd)
  ...
  !
  RETURN
END SUBROUTINE parity_band
!
SUBROUTINE find_parity_sym_so (ik,evc,et,nsym,s,ft,d_spin,gk, &
     invs,rap_par,times,ngroup,istart,accuracy)
  ...
  DO iclass=1,nclass
    ...
    IF ( all(s(:,:,irot) == -s(:,:,1)) ) THEN
      IF ( has_e(1,iclass) == 1 ) THEN
        ...
        DO ibnd =1,nbnd
             trace(iclass,ibnd)=trace(iclass,ibnd) +            &
  ...
  ! WRITE (stdout, '(/,1x,i8,74("*"))') ik
  ! WRITE (stdout, '(10x,10f9.5)') (REAL(trace(iclassI,ibnd)), ibnd=1,nbnd)

  DO ibnd =1,nbnd
    IF (iclassI == -1) THEN
      rap_par(ibnd) = 0
    ELSE
      IF (( ABS( 1 - ABS(REAL(trace(iclassI,ibnd))))) < 1.0d-5 ) THEN
          rap_par(ibnd) = NINT(REAL(trace(iclassI,ibnd)))
      ELSE
          rap_par(ibnd) = 0
      ENDIF
    ENDIF
  ...
  !    DO i=1,dim_rap
  !       ibnd=istart(igroup)+i-1
  !       IF (( ABS( 1 - ABS(REAL(trace(iclassI,igroup))))) < 1.0d-5 ) THEN
  !          rap_par(ibnd) = NINT(REAL(trace(iclassI,igroup)))
  !       ELSE
  !          rap_par(ibnd) = 0
  !       ENDIF
  !    ENDDO
  ...
  RETURN
END SUBROUTINE find_parity_sym_so
!
