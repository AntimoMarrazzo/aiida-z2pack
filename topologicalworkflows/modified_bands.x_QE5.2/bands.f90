...
PROGRAM do_bands
  ...
  LOGICAL :: lsigma(4), lsym, lp, no_overlap, plot_2d, parity
  INTEGER :: spin_component, firstk, lastk
  INTEGER :: ios
  !
  NAMELIST / bands / outdir, prefix, filband, filp, spin_component, lsigma,&
                       lsym, parity, lp, filp, firstk, lastk, no_overlap, plot_2d
  !
  ...
  CALL mp_bcast( parity, ionode_id, world_comm )
  ...
  ELSE
     ...
     IF (parity) CALL parity_band(filband,spin_component,firstk,lastk)
  END IF
  !
