...
SUBROUTINE parity_band(filband, spin_component, firstk, lastk)
  ...
  DO ik = nks1, nks2
     ...
     IF (invsymk) THEN
           CALL find_parity_sym_so(evc,et(1,ik),nsymk,sk,ftauk,d_spink,&
                gk,rap_par(1,ik),times(1,1,ik),ngroup(ik),&
                istart(1,ik),accuracy)
     ENDIF
...
SUBROUTINE find_parity_sym_so (evc,et,nsym,s,ftau,d_spin,gk, &
     rap_par,times,ngroup,istart,accuracy)
